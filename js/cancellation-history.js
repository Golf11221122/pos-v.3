...
async function requireUser(){
  ...
  const {data,error:pe}=await supabase.from('profiles').select('id,full_name,role,branch_id,branches(name)').eq('id',session.user.id).single()
  if(pe) throw pe
  if(!['admin','manager'].includes(String(data.role||'').toLowerCase())) throw new Error('หน้านี้สำหรับ Admin / Manager')
  el.branchText.textContent = data.branches?.name ? `สาขา ${data.branches.name}` : 'รายงานสาขาปัจจุบัน'
}
