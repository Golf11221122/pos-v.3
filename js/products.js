...
const elements = {
    ...
    inactiveProductCount: document.getElementById('inactiveProductCount'),
    ...
    productBranch: document.getElementById('productBranch'),
...
imageFileName: document.getElementById('imageFileName'),
    ...
    salesMenu: document.getElementById('salesMenu'),
    ingredientsMenu: document.getElementById('ingredientsMenu'),
...
function createImagePath(file) {
    ...
    const branchId = state.profile?.branch_id || 'unassigned'
    const uniqueId = crypto.randomUUID()

    return `${branchId}/${uniqueId}.${extension}`
}
...
function validateImageFile(file) {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
        return 'รูปสินค้าต้องมีขนาดไม่เกิน 5 MB'
    }

    return null
}

function showImagePreview(source, fileName = '') {
    elements.imagePreviewElement.src = source
    elements.imagePreviewElement.classList.remove('hidden')
    ...
    elements.imageFileName.textContent =
        fileName || 'มีรูปสินค้าแล้ว'
}

function clearImagePreview() {
    elements.imagePreviewElement.removeAttribute('src')
    elements.imagePreviewElement.classList.add('hidden')
...
async function uploadProductImage(file) {
    ...
    if (!data?.publicUrl) {
        await deleteProductImageByPath(filePath)

        throw new Error('ไม่สามารถสร้าง URL รูปสินค้าได้')
    }
...
function getStoragePathFromPublicUrl(publicUrl) {
    ...
    try {
        ...
        console.warn('Invalid product image URL:', error)
        return null
    }
}

async function deleteProductImageByPath(filePath) {
    if (!filePath) {
        return
    }
...
async function deleteProductImageByUrl(publicUrl) {
...
async function loadProfile(userId) {
    ...
    if (!data) {
        throw new Error('ไม่พบข้อมูลผู้ใช้งานในตาราง profiles')
    }
...
async function loadBranches() {
    ...
    state.branch =
        state.branches.find(
            branch => branch.id === state.profile?.branch_id
        ) || null
}
...
async function loadCategories() {
    ...
    if (state.profile?.branch_id) {
        query = query.eq('branch_id', state.profile.branch_id)
    }
...
async function loadProducts() {
    showLoading()
    setPageMessage('กำลังโหลดสินค้า...', 'info')
...
function renderUser() {
    ...
    const role = formatRole(state.profile?.role)
    const branchName = state.branch?.name || 'ยังไม่ได้กำหนดสาขา'
    ...
    elements.branchLabel.textContent = `สาขา: ${branchName}`
}
...
function populateBranchOptions() {
    ...
    elements.productBranch.innerHTML = options.join('')
...
function updateSummary() {
    ...
    elements.activeProductCount.textContent =
        active.toLocaleString('th-TH')

    elements.inactiveProductCount.textContent =
        inactive.toLocaleString('th-TH')

    elements.categoryCount.textContent =
        state.categories.length.toLocaleString('th-TH')
}
...
function renderProducts() {
    ...
    elements.resultText.textContent =
        `พบ ${products.length.toLocaleString('th-TH')} รายการ`
    ...
    if (products.length === 0) {
        ...
        elements.emptyState.querySelector('h3').textContent =
            hasFilter ? 'ไม่พบสินค้าที่ค้นหา' : 'ยังไม่มีสินค้า'
...
function createProductRow(product) {
    ...
    const toggleTitle = product.is_active
        ? 'ปิดการขาย'
        : 'เปิดการขาย'
        ...
        <tr>
            ...
            <td>
                <div class="code-cell">
                    ...
                    <span>Barcode: ${escapeHtml(barcode)}</span>
                </div>
            </td>
...
function openAddModal() {
    ...
    elements.modalTitle.textContent = 'เพิ่มสินค้า'
    elements.saveProductBtn.textContent = 'บันทึกสินค้า'
...
function openEditModal(productId) {
    ...
    if (!product) {
        setPageMessage('ไม่พบข้อมูลสินค้า')
        return
    }

    resetProductForm()

    elements.modalTitle.textContent = 'แก้ไขสินค้า'
    elements.saveProductBtn.textContent = 'บันทึกการแก้ไข'

    elements.productId.value = product.id
    elements.productName.value = product.name || ''
    elements.productCategory.value = product.category_id || ''
    elements.productBranch.value = product.branch_id || ''
    elements.productPrice.value = Number(product.price || 0)
    elements.productCost.value = Number(product.cost || 0)
    elements.productSku.value = product.sku || ''
    elements.productBarcode.value = product.barcode || ''
    elements.productStock.value = Number(product.stock || 0)
...
function validateProductForm() {
    ...
    const branchId = elements.productBranch.value

    const price = Number(elements.productPrice.value)
    const cost = Number(elements.productCost.value || 0)
    const stock = Number(elements.productStock.value || 0)
    ...
    if (
        ...
        return 'ราคาขายต้องเป็นตัวเลขตั้งแต่ 0 ขึ้นไป'
    }
    ...
    if (
        ...
        return 'จำนวนคงเหลือต้องเป็นตัวเลขตั้งแต่ 0 ขึ้นไป'
    }

    return null
}
function getProductPayload(imageUrl = null) {
    return {
        name: elements.productName.value.trim(),
        category_id: elements.productCategory.value,
        branch_id: elements.productBranch.value,
        ...
        description:
            elements.productDescription.value.trim() || null,

        display_order:
            Number(elements.displayOrder.value || 0),

        is_active: elements.productActive.checked
    }
}
...
async function saveProduct(event) {
    ...
    const oldImageUrl = state.currentImageUrl
    ...
    try {
        if (state.selectedImageFile) {
            newUploadedImage = await uploadProductImage(
                state.selectedImageFile
            )

            finalImageUrl = newUploadedImage.publicUrl

            elements.saveProductBtn.textContent =
                'กำลังบันทึกข้อมูล...'
        } else if (state.removeCurrentImage) {
            finalImageUrl = null
        }

        const payload = getProductPayload(finalImageUrl)
        ...
        /*
         * ลบรูปเก่าหลังจากบันทึกฐานข้อมูลสำเร็จแล้วเท่านั้น
         * เพื่อป้องกันสินค้าสูญเสียรูปเมื่อการบันทึกล้มเหลว
         */
        const imageWasReplaced =
            Boolean(newUploadedImage) &&
            Boolean(oldImageUrl) &&
        ...
        /*
         * อัปโหลดรูปสำเร็จ แต่บันทึกฐานข้อมูลล้มเหลว
         * จึงลบรูปใหม่เพื่อไม่ให้มีไฟล์ค้างใน Storage
         */
        if (newUploadedImage?.filePath) {
            await deleteProductImageByPath(
                newUploadedImage.filePath
            )
        }

        let message =
            error.message || 'บันทึกสินค้าไม่สำเร็จ'
...
async function deleteProduct() {
    const productId = state.deleteProductId
    ...
    try {
        ...
        closeDeleteModal()
        setPageMessage(`ลบสินค้าไม่สำเร็จ: ${message}`)
...
elements.selectImageBtn.addEventListener(
    'click',
    () => {
...
elements.productImageFile.addEventListener(
    'change',
    handleImageSelection
)

elements.removeImageBtn.addEventListener(
    'click',
    handleRemoveImage
)

elements.addProductBtn.addEventListener(
    'click',
    openAddModal
)
...
elements.categoriesMenu.addEventListener('click', event => {
    event.preventDefault()
    showComingSoon('หน้าจัดการหมวดหมู่')
})

elements.salesMenu.addEventListener('click', event => {
    event.preventDefault()
    showComingSoon('หน้ารายการขาย')
})

elements.ingredientsMenu.addEventListener('click', event => {
    event.preventDefault()
    showComingSoon('หน้าจัดการวัตถุดิบ')
})

elements.reportsMenu.addEventListener('click', event => {
    event.preventDefault()
